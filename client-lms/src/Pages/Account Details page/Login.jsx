...
const Login = () => {
  ...
  //fetch the login from the auth slice
  const logIn = async (e) => {
    e.preventDefault();
    ...
    <>
      <div className="flex items-center h-screen w-screen justify-center bg-login bg-cover bg-no-repeat">
        <div className=" backdrop-blur-sm h-screen w-screen flex items-center justify-center">
          <div className="flex flex-col gap-3 items-center justify-between py-5 min-h-fit md:h-[70%] bg-white w-[85%] md:w-2/5 animate-slide-in-left">
            <h1 className="uppercase font-light tracking-[.35em] sm:text-3xl font-serif text-[#269d8b]">
              LogIn
            </h1>
            <form
              onSubmit={logIn}
              className=" flex flex-col gap-3 items-center justify-evenly w-[80%] h-[70%] "
            >
